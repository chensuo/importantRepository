...
$(function(e){
    ...
    * */
    $('.btnSaveSingerCategory').bind('click',function(e){
        ...
        $.ajax({
            type:'post',
            url:'http://101.200.58.3/htmlprojectwebapi/SingerRegion/create',
            data:{name:addNewCategoryInput.val(),accountId:currentUserData.Id},
            success:function(result){
                    if(result.Code == 100){
                        //console.log(catagory);
                        var $tr = createSingerCategory(result.Data,countNew);
            ...
            success:function(result){
                if(result.Code == 100){
                    ...
                    for(var i = list.length - 1 ; i >= 0 ; i--){
                        var $tr = createSingerCategory(list[i],count);
    ...
    * 创建歌手类别项目
    * */
    function createSingerCategory(dataItem,num){
        count++;
        var $tr = $('<tr></tr>');
        ...
        $btnChange.bind('click',function(e){
            ...
            $btnSaveChange.bind('click',function(e){
                ...
                $.ajax({
                    type:'post',
                    url:'http://101.200.58.3/htmlprojectwebapi/SingerRegion/update',
                    data:{id:dataItem.Id,name:changeInput.val(),accountId:currentUserData.Id},
                    success:function(result){
                        if(result.Code == 100){
                            console.log(result.Data);
                            $(self).parent().parent().replaceWith(createSingerCategory(result.Data, num));
